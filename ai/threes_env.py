...
class ThreesEnv(py_environment.PyEnvironment):
    ...
    def _reset(self):
        state = self.connector.restart()
        return ts.restart(self.__state_to_observation(state))

    def _step(self, action):
        state = self.connector.shift(action.item(0))

        if state['GameOver']:
            return ts.termination(self.__state_to_observation(state), reward=state['Score'])
        else:
            return ts.transition(self.__state_to_observation(state), reward=self.__score_board(state))
